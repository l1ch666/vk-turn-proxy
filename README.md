...
`-vless-bond` is an explicit VLESS-only mode for full-tunnel clients that keep several TURN/DTLS streams alive (`-n 10`, for example). It requires `-vless` on both sides:
